...
function isInsideCircle(circle, point) {
  ...
  return Math.sqrt((x1 - point.x) ** 2 + (y1 - point.y) ** 2) < circle.radius;
}
...
function isCreditCardNumber(ccn) {
  const array = ccn.toString().split('').map((el, i, arr) => {
    let x = Number(el);
    if ((arr.length - i) % 2 === 0) {
      x *= 2;
    }
  ...
  return array.reduce((a, b) => a + b) % 10 === 0;
}
...
function getDigitalRoot(n) {
  const sum = n % 10;
  const res = Math.trunc(n / 10) !== 0 ? sum + getDigitalRoot(Math.trunc(n / 10)) : sum;
  return `${res}`.length !== 1 ? getDigitalRoot(res) : res;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  arr[0].forEach((item, i) => {
    if (arr.every((el) => el[i] === item)) res.push(item);
  });
...
function getMatrixProduct(A, B) {
  ...
  const res = new Array(rowA);
  if (colA !== rowB) return false;
  for (let k = 0; k < rowA; k += 1) {
    res[k] = new Array(colB);
    for (let i = 0; i < colB; i += 1) {
      res[k][i] = 0;
      for (let j = 0; j < colA; j += 1) {
        res[k][i] += A[k][j] * B[j][i];
      }
    }
  }
